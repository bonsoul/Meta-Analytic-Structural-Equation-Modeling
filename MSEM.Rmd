...
model2 <- mxModel("Mediation",
  ...
  mxPath(from=c("X","M","Y"), arrows=2, free=TRUE, values=1) # variances
)
```
...
# Step 1: Correlation matrices with names
cor1 <- matrix(c(1, .4, .3,
                 .4, 1, .5,
                 .3, .5, 1), nrow=3, byrow=TRUE,
               dimnames=list(c("X","M","Y"), c("X","M","Y")))
...
# Step 2: Stage 1 random-effects MASEM
stage1 <- tssem1(cors, n, method="REM")

# Step 3: Specify SEM model in RAM notation
model2 <- mxModel("Mediation",
  type="RAM",
  manifestVars=c("X","M","Y"),
  mxPath(from="X", to="M", arrows=1, free=TRUE, values=0.2, labels="a"),
